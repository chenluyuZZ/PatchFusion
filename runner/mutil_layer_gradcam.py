...
class Gradmodel():
    ...
    def get_cam_mask(self): # 8 16 1 224 224
        ...
        self.tensor_cam = torch.concat(self.cam,dim = 1) # 每张图的cam 合并在一起 16 8 224 224 
...
for i,(img,label,index) in enumerate(dataloader_list[0]):
    ...
    for k in range(8):
        ...
        for j in range(8):
            img_add_cam,heatmap =superimpose(img[k].cpu().numpy(),grad_model.tensor_cam[k][j].cpu().numpy(),0.5, f'/home/sstl/fht/mask_train/sample/multi_cam/gray/{k}_img_{j}.png' ) # 3 224 224  224 224 
            
            cv2.imwrite(f'/home/sstl/fht/mask_train/sample/multi_cam/cam/{k}_img_{j}.png',img_add_cam)
        all_layer = grad_model.tensor_cam[k].mean(dim=0)
        all_add_cam, heatmap = superimpose(img[k].cpu().numpy(),all_layer.cpu().numpy(),0.5,f'/home/sstl/fht/mask_train/sample/multi_cam/gray/{k}_all_layer.png')
        cv2.imwrite(f'/home/sstl/fht/mask_train/sample/multi_cam/cam/{k}_all_layer.png',all_add_cam)
    if i == 0 :
        break
