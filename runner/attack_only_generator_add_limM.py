...
class pgd_attack():
    ...
    def attack(self):
        ...
        for i,(images,label,index) in enumerate(Data['dataloader']['train']):
            ...
            g_mask,cor_list = self.generator.dilation_Erosion() # 限制为 500-5000
            ...
            for j in range(self.iters):
                ...
                delta.data = (delta + self.alpha * delta.grad.detach().sign()).clamp(self.epsilon[0]*0.3, self.epsilon[1]*0.3)
                delta.grad.zero_()
            ...
            for k in cor_list:
                save_images(unorm(attack_img.detach()[k]).unsqueeze(0),[f'{index[k]}_ori_label{label[k]}_preds{preds[k]}.png'],'/home/sstl/fht/mask_train/runner/attack_g_add_lim')
                save_images(mask[k].unsqueeze(0).unsqueeze(0),[f'{index[k]}_mask.png'],'/home/sstl/fht/mask_train/runner/attack_g_add_lim')
